...
class RefCOCOMapper:
    ...
    def from_config(cls, cfg, is_train=True):
        ...
        ret = {
            ...
            "pickle_dir": cfg.REFERRING.SAM_PICKLE_DIR,
    ...
    def __call__(self, dataset_dict):
        ...
        dataset_dict["orig_shape"] = orig_shape
        ...
        if self.use_instances_annos:
            annos_coco = [ utils.transform_instance_annotations(obj, transforms, image_shape)
                for obj in dataset_dict.pop("coco")
                if (obj.get("iscrowd", 0) == 0) and (obj.get("empty", False) == False)]
            instances_coco = utils.annotations_to_instances(annos_coco, image_shape)
            if len(instances_coco) > 0:
                assert (not empty)
                instances_coco.gt_boxes = instances_coco.gt_masks.get_bounding_boxes()
                # Generate masks from polygon
                h, w = instances_coco.image_size
                assert hasattr(instances_coco, 'gt_masks')
                gt_masks_coco = instances_coco.gt_masks
                gt_masks_coco = convert_coco_poly_to_mask(gt_masks_coco.polygons, h, w)
                instances_coco.gt_masks = gt_masks_coco
            else:
                assert empty
                gt_masks_coco = torch.zeros((0, image_shape[0], image_shape[1]), dtype=torch.uint8)
                instances_coco.gt_masks = gt_masks_coco      
            if self.is_train:
                dataset_dict["instances_coco"] = instances_coco
            else:
                dataset_dict["gt_mask_coco"] = gt_masks_coco # これ何に使うの？

        if self.use_pickle:
            pickle_fn = os.path.basename(dataset_dict["file_name"])
            pickle_fn,_ = os.path.splitext(pickle_fn)
            pickle_path = os.path.join(self.pickle_dir, pickle_fn + '.pickle')
            with open(pickle_path, 'rb') as fr:
               sam_embeds = pickle.load(fr)
            dataset_dict["sam_embs"] = sam_embeds
        else:
            sam_embeds = None  

        dataset_dict["caption"] = sentence_raw
        #dataset_dict["coco_class"] = dataset_dict['coco_class'] # NOTE なんか気持ち悪い消したい
        return dataset_dict



    def __call__old(self, dataset_dict):
        dataset_dict = copy.deepcopy(dataset_dict)  # it will be modified by code below
        image = utils.read_image(dataset_dict["file_name"], format=self.img_format)
        utils.check_image_size(dataset_dict, image)
